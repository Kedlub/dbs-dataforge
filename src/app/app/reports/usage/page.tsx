...
const usageReportSchema = z
	...
	.refine((data) => data.endDate >= data.startDate, {
		...
		path: ['endDate'] // Set error path to endDate field
	});
...
export default function UsageReportPage() {
	...
	const onSubmit = async (data: UsageReportFormData) => {
		...
		try {
			...
			toast.success('Report využití byl úspěšně zařazen ke generování.');
			router.push('/app/reports'); // Redirect back to the main reports page
		} catch (error) {
	...
	return (
		<Card>
			<CardHeader>
				<CardTitle>Generovat Report Využití</CardTitle>
				<CardDescription>
					Zadejte parametry pro nový report statistik využití.
				</CardDescription>
			</CardHeader>
			<CardContent>
				<Form {...form}>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Input placeholder="Např. Využití Leden 2025" {...field} />
									</FormControl>
									<FormMessage />
								</FormItem>
							)}
